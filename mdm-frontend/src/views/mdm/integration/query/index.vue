  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" v-model="open" width="520px" append-to-body>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="接口编码" prop="apiCode">
          <el-input v-model="form.apiCode" :disabled="form.id != null" placeholder="对外路径使用，如 customer-query" />
        </el-form-item>
...
import type { MdmQueryApi, MdmObject } from '@/types'
import { listQuery, addQuery, editQuery, delQuery } from '@/api/mdm/integration'
import { listObject } from '@/api/mdm/model'

const loading = ref(false)
const list = ref<MdmQueryApi[]>([])
const total = ref(0)
const objectOptions = ref<MdmObject[]>([])
const queryParams = ref({ pageNum: 1, pageSize: 10, apiCode: '', objectCode: '', status: '' })
const open = ref(false)
const title = ref('')
const form = ref<MdmQueryApi>({ status: '0' })
...
const getList = async () => {
  loading.value = true
  try {
    const res = await listQuery(queryParams.value)
    list.value = res.rows
...
const handleEdit = (row: MdmQueryApi) => {
  form.value = { ...row }
  title.value = '编辑查询接口'
  open.value = true
}
...
const submitForm = () => {
  formRef.value?.validate(async (valid) => {
    ...
    if (form.value.id != null) {
      await editQuery(form.value)
    } else {
      await addQuery(form.value)
    }
...
const handleDelete = (row: MdmQueryApi) => {
  ElMessageBox.confirm(`确定删除查询接口「${row.apiName}」吗？`, '系统提示', { type: 'warning' })
    .then(async () => {
      await delQuery(row.id!)
