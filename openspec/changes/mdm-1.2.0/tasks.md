...
- [x] 4.1 前后端联调：集成管理 5 菜单全流程走查（应用注册 → 配置接收/查询接口 → 分发 → 日志可查）（工时：3 人天）
- [x] 4.2 回归：`scripts/smoke-mdm.sh` 全量 + 本版本 6 项问题逐一复验（工时：2 人天）
- [x] 4.3 文档：`doc/RELEASES.md` 登记 1.2.0 发布记录；`doc/operations.md` 补充集成管理运维说明（存量库执行 upgrade-1.2.0.sql）（工时：1 人天）
- [x] 4.4 发布：打 tag `v1.2.0`（工时：0.5 人天）
